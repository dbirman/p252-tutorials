...
ggplot(data=rtw.table, aes(Income,COL)) +
  geom_point(size=3) +
...
```{r lm_add_rtw.table}
rs_add = lm(COL ~ RTWL + IncomeCat, data = rtw.table)
summary(rs_add)
```
...
rs_int = lm(COL ~ RTWL * IncomeCat, data = rtw.table)
summary(rs_int)
```
...
ggplot(data=rtw.table, aes(x=RTWL,y=COL,color=IncomeCat,group=IncomeCat)) +
